...
def creation_model(Tau_MSN = 30, Tau_FSI = 30, Tau_STN = 30, Tau_GPe = 30, Tau_GPiSNr = 30 , Dt = 1, time = 1000.0, NbChannels = 2, Lambda = 0, seed = 456, show_input = False, show_firing = True, show_proba = False, show_all = None, save = False, method_stim = 'random', inputCh1 = 1, inputCh2 = 1):
    if show_all != None:
...
#### MODELE rBCBG AVEC ANNarchy ####
    ...
    stimuli_CSN = creation_stimuli(NbChannels, laps, time, num_seed = seed, method = method_stim, nucleus = 'CSN', inputCh1 = inputCh1, inputCh2 = inputCh2 )
    stimuli_PTN = creation_stimuli(NbChannels, laps, time, num_seed = seed, method = method_stim, nucleus = 'PTN', inputCh1 = inputCh1, inputCh2 = inputCh2 )
...
def creation_stimuli(NbChannels,laps,time,num_seed = 456, method = 'manual', nucleus = 'CSN', inputCh1 = 1, inputCh2 = 1):
    stimuli = {}
    ...
    if method == 'manual':
        if nucleus == 'CSN':
            ...
            stim = [ [[20*inputCh1,time,0,laps]] , [[20*inputCh2,time,0,laps]] ]
        if nucleus == 'PTN':
            ...
            stim = [ [[45*inputCh1,time,0,laps]] , [[45*inputCh2,time,0,laps]] ]
    if method == 'random':
        rd.seed(num_seed)
        if nucleus == 'CSN':
...
#affiche la variation de l'entropie de la PDF en fonction de lambda, comme dans l'article Khamassi Humphries 2012,
# si save = true : les sauvegarde (dans log)
def Khamassi_var_entropy(nb_points = 11,lambda_max = 1, nb_inputs = 10, first_seed = 456, save = False):
    list_lambda = []
    list_H = []
    list_varH = []
...
# affiche les figures de densités de probas en fonction des variations de lambda, comme  dans l'article Khamassi Humphries 2012, 
# si save = true : les sauvegarde (dans log)
def Khamassi_var_proba(save = False):
    for i in [0,0.5,1]:
        creation_model(Lambda=i, show_all = True, method_stim = 'manual', save = save)


def Khamassi_reinforcment():

    #initialisation des Q-values à 0 
    signes = {'a':0.8,'b':0.2,'c':0.7,'d':0.3,'e':0.6,'f':0.4}
    Qvalue = {'a':0.5,'b':0.5,'c':0.5,'d':0.5,'e':0.5,'f':0.5}
    alpha = 0.1

    #création d'une liste de 360 inputs randomisé (pour la tache de Mehdi) AB, CD ou EF
    nb_repet = 5
    inputsCh1 = ['a' for i in range(nb_repet) ] #+ [ 'c' for i in range(nb_repet) ] + [ 'e' for i in range(nb_repet)]
    rd.Random(123).shuffle(inputsCh1)
    inputsCh2 = copy(inputsCh1)
    for idx in range(len(inputsCh1)):
        if inputsCh2[idx] == 'a':
            inputsCh2[idx] = 'b'
        if inputsCh2[idx] == 'c':
            inputsCh2[idx] = 'd'
    ...
    varQvalue = []
    var_probas = []
    varQvalue.append(list(Qvalue.values()))
    #On fait tourner creation_model avec un lambda fixé pour tous les inputs de la liste (360fois)
    for i in range(len(inputsCh1)):
        signe1, signe2 = inputsCh1[i], inputsCh2[i]
        input1, input2 = Qvalue[signe1], Qvalue[signe2]
        #On récupère la PDF donné par le modèle 
        H,probas = creation_model(Lambda = 0, show_all = False, method_stim = 'manual', inputCh1 = input1, inputCh2 = input2)
        #On tire au sort, selon cette distribution, le choix de l'action 1 ou de l'action 2
        if rd.random() <= probas[0]:
            choix = signe1
        else:
            choix = signe2
        #On attribue le reward 
        if rd.random() <= signes[choix]:
            reward = 1
        else:
            reward = 0
        #On met à jour la Q-value en question 
        Qvalue[choix] = Qvalue[choix] + alpha*(reward-Qvalue[choix])
        varQvalue.append(list(Qvalue.values()))
        var_probas.append(probas)
...
def probas():
    ...
    # deltaLvls = []
    # for delta in deltaLvls:
    deltaLvl = 0.1
    for input in levels:
        H,probas = creation_model(inputCh1= input+deltaLvl, inputCh2= input)
        Lprobas.append(probas)
    print(probas)
##########################################################################################################################
...
if __name__ == '__main__':
    ...
    # creation_model(Lambda = 0, show_firing=False,show_all = True, method_stim='manual')
    #Khamassi_var_proba()
    #Khamassi_var_entropy(nb_points = 2, lambda_max = 1, nb_inputs = 2, first_seed = 456)
    #Khamassi_reinforcment()
    probas()
...
#Questions:
#On s'en fiche de changer de donner A (C et E) toujours au même canal ? oui...
#La première valeur c'est pas grave si c'est 0 ? 
