...
def creation_model(Tau_MSN = 30, Tau_FSI = 30, Tau_STN = 30, Tau_GPe = 30, Tau_GPiSNr = 30 , Dt = 1, time = 1000.0, NbChannels = 4, show_input = False):
    ...
    ### BG ###
    FSI = Population(name='FSI', geometry=NbChannels, neuron=LeakyIntegratorNeuron_FSI) # a diviser en 2 aussi aussi ? 
...
###############
    ...
    proj_CSN1_MSNd1 = Projection(pre=CSN1, post=MSNd1[0], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1+lambda1) , delays = latency['dCSN_MSN'])
    proj_CSN1_MSNd2 = Projection(pre=CSN1, post=MSNd2[0], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1-lambda2) , delays = latency['dCSN_MSN']) 
    ...
    proj_PTN1_MSNd1 = Projection(pre=PTN1, post=MSNd1[0], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1+lambda1) , delays = latency['dPTN_MSN']) 
    proj_PTN1_MSNd2 = Projection(pre=PTN1, post=MSNd2[0], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1-lambda2) , delays = latency['dPTN_MSN'])
    ...
    proj_CSN2_MSNd1 = Projection(pre=CSN2, post=MSNd1[1], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1+lambda1) , delays = latency['dCSN_MSN']) 
    proj_CSN2_MSNd2 = Projection(pre=CSN2, post=MSNd2[1], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1-lambda2) , delays = latency['dCSN_MSN']) 
    ...
    proj_PTN2_MSNd1 = Projection(pre=PTN2, post=MSNd1[1], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1+lambda1) , delays = latency['dPTN_MSN']) 
    proj_PTN2_MSNd2 = Projection(pre=PTN2, post=MSNd2[1], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1-lambda2) , delays = latency['dPTN_MSN']) 
    ...
    proj_CSN3_MSNd1 = Projection(pre=CSN3, post=MSNd1[2], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1+lambda1) , delays = latency['dCSN_MSN']) 
    proj_CSN3_MSNd2 = Projection(pre=CSN3, post=MSNd2[2], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1-lambda2) , delays = latency['dCSN_MSN']) 
    ...
    proj_PTN3_MSNd1 = Projection(pre=PTN3, post=MSNd1[2], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1+lambda1) , delays = latency['dPTN_MSN']) 
    proj_PTN3_MSNd2 = Projection(pre=PTN3, post=MSNd2[2], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1-lambda2) , delays = latency['dPTN_MSN']) 
    ...
    proj_CSN4_MSNd1 = Projection(pre=CSN4, post=MSNd1[3], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1+lambda1) , delays = latency['dCSN_MSN']) 
    proj_CSN4_MSNd2 = Projection(pre=CSN4, post=MSNd2[3], target='exc', synapse=Basic).connect_one_to_one(weights= W_CSN_MSN*(1-lambda2) , delays = latency['dCSN_MSN']) 
    ...
    proj_PTN4_MSNd1 = Projection(pre=PTN4, post=MSNd1[3], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1+lambda1) , delays = latency['dPTN_MSN']) 
    proj_PTN4_MSNd2 = Projection(pre=PTN4, post=MSNd2[3], target='exc', synapse=Basic).connect_one_to_one(weights= W_PTN_MSN*(1-lambda2) , delays = latency['dPTN_MSN']) 
...
################
    ...
    rGPi = view_all_firing_rate(NbChannels, MSNd1monitors, MSNd2monitors, STNmonitors,FSImonitors,GPemonitors,GPiSNrmonitors, all_time, Dt = Dt, Tau_MSN = Tau_MSN, Tau_FSI = Tau_FSI, Tau_STN = Tau_STN, Tau_GPe = Tau_GPe, Tau_GPiSNr = Tau_GPiSNr )
...
def view_all_firing_rate( NbChannels, MSNd1monitors, MSNd2monitors, STNmonitors,FSImonitors,GPemonitors,GPiSNrmonitors, all_time, projection_test = False, Shapley_test = False, Dt = 1, test_variation_tau = False, Tau_MSN = 5, Tau_FSI = 5, Tau_STN = 5, Tau_GPe = 5, Tau_GPiSNr = 5):
    ...
    for idx,monitor in enumerate(GPiSNrmonitors):
        ...
        if idx <= 1: #on récupere le firing rate du GPi pour les deux premiers canaux : le premier étant celui stimuler et l'autre pour la comparaison
            rGPi.append(r)
    axs[5,0].axes.set_ylabel("GPiSNr",size = 16)
