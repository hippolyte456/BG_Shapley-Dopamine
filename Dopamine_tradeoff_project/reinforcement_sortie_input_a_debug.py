...
def creation_model(inpCSN,inpPTN, Tau_MSN = 30, Tau_FSI = 30, Tau_STN = 30, Tau_GPe = 30, Tau_GPiSNr = 30 , Dt = 1, time = 1000.0, NbChannels = 2, Lambda = 0, seed = 456, show_input = False, show_firing = True, show_proba = False, show_all = None, save = False):
    if show_all != None:
...
def creation_stimuli(NbChannels,laps,time,num_seed = 456, method = 'manual', nucleus = 'CSN',):
    ...
    if method == 'manual':
        if nucleus == 'CSN':
            stim = [ [[2,time,0,laps]],[[5,time,0,laps]] ]
        if nucleus == 'PTN':
            stim = [ [[15,time,0,laps]],[[20,time,0,laps]] ]    
...
def creation_TimedArray(time,NbCanaux,funct,stimuli):  #*args
    ...
    #print('sortie timed array = ' , inp)
    return (inp)


def creation_inputs(NbChannels = 2, time = 1000, seed = 456, method_stim = 'manual'):
    laps = 30  
    ...
    #print('sortie creation input = ' , CSN)
    return CSN, PTN
...
#     a = time.time()
#     creation_model(Lambda = 0, show_firing=False,show_all = True)
#     #Khamassi_var_proba()
...
if __name__ == '__main__':
    ...
    Qvalue = {'a':0,'b':0,'c':0,'d':0,'e':0,'f':0}
    ...
    #On fait tourner creation_model avec un lambda fixé en donnant le premier input de la liste 
    signe1, signe2 = inputsCh1[0], inputsCh2[0]
    input1, input2 = Qvalue[signe1], Qvalue[signe2]
    CSN = creation_inputs()[0]
    PTN = creation_inputs()[1]
    print(CSN)
    creation_model(inpCSN= CSN, inpPTN= PTN, Lambda = 0, show_all = True)
